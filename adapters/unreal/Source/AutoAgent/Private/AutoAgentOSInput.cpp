...
void FAutoAgentOSInput::Click(FVector2D ScreenPos, const FString& Button)
{
	...
	DWORD Down, Up;
	if      (Button == TEXT("right"))  { Down = MOUSEEVENTF_RIGHTDOWN;  Up = MOUSEEVENTF_RIGHTUP;  }
	else if (Button == TEXT("middle")) { Down = MOUSEEVENTF_MIDDLEDOWN; Up = MOUSEEVENTF_MIDDLEUP; }
...
int32 FAutoAgentOSInput::MapKey(const FString& Key)
{
	...
	if      (K == TEXT("enter") || K == TEXT("return") || K == TEXT("submit")) return VK_RETURN;
	else if (K == TEXT("escape") || K == TEXT("esc") || K == TEXT("cancel"))   return VK_ESCAPE;
	else if (K == TEXT("tab"))     return VK_TAB;
	else    return -1;
}
...
{
	if (Evt) { CGEventPost(kCGHIDEventTap, Evt); CFRelease(Evt); }
}
...
void FAutoAgentOSInput::Click(FVector2D ScreenPos, const FString& Button)
{
	...
	CGEventType Down, Up;
	CGMouseButton MBtn;
	if      (Button == TEXT("right"))  { Down = kCGEventRightMouseDown; Up = kCGEventRightMouseUp; MBtn = kCGMouseButtonRight;  }
	else if (Button == TEXT("middle")) { Down = kCGEventOtherMouseDown; Up = kCGEventOtherMouseUp; MBtn = kCGMouseButtonCenter; }
	else                               { Down = kCGEventLeftMouseDown;  Up = kCGEventLeftMouseUp;   MBtn = kCGMouseButtonLeft;   }
...
void FAutoAgentOSInput::Scroll(FVector2D ScreenPos, float Delta)
{
	CGPoint Pt = ToCGPoint(ScreenPos);
	PostAndRelease(CGEventCreateMouseEvent(nullptr, kCGEventMouseMoved, Pt, kCGMouseButtonLeft));
	PostAndRelease(CGEventCreateScrollWheelEvent(nullptr, kCGScrollEventUnitPixel, 1,
		static_cast<int32>(Delta * 3)));
}
...
{
	FString K = Key.ToLower().TrimStartAndEnd();
	if      (K == TEXT("enter") || K == TEXT("return") || K == TEXT("submit")) return CGKEY_RETURN;
	else if (K == TEXT("escape") || K == TEXT("esc") || K == TEXT("cancel"))   return CGKEY_ESCAPE;
	else if (K == TEXT("tab"))     return CGKEY_TAB;
	else    return -1;
}
...
static void InitX11()
{
	...
	*(void**)&GXTestFakeKeyEvent    = dlsym(Xtst, "XTestFakeKeyEvent");
	...
	if (GXOpenDisplay) GX11Display = (Display*)GXOpenDisplay(nullptr);
	if (GX11Display)   GX11Screen  = GXDefaultScreen ? GXDefaultScreen(GX11Display) : 0;
}
...
void FAutoAgentOSInput::Click(FVector2D ScreenPos, const FString& Button)
{
	...
	unsigned int Btn;
	if      (Button == TEXT("right"))  Btn = 3;
	else if (Button == TEXT("middle")) Btn = 2;
	else                               Btn = 1;
...
{
	FString K = Key.ToLower().TrimStartAndEnd();
	if      (K == TEXT("enter") || K == TEXT("return") || K == TEXT("submit")) return XK_RETURN;
	else if (K == TEXT("escape") || K == TEXT("esc") || K == TEXT("cancel"))   return XK_ESCAPE;
	else if (K == TEXT("tab"))     return XK_TAB;
	else    return -1;
}
