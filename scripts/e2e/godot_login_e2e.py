"""End-to-end login flow test for the Godot adapter (TASK-0306).

Connects to a running AutoAgent Godot adapter, exercises the full login flow
(send_text → click → assert visibility) and runs the visual regression check
via compare_screenshot.

Prerequisites
-------------
  1. Godot fixture project running (LoginScene, main loop active)
  2. AutoAgent plugin enabled (server on ws://127.0.0.1:27842)
  3. pip install websockets jsonschema

Usage
-----
    python scripts/e2e/godot_login_e2e.py [--url ws://127.0.0.1:27842]
...
# Stable IDs expected in the Godot login fixture (mirrors Unity + UE fixtures).
EXPECTED_PINNED = {
    "login_panel",
    "account_input_bg",
    "password_input_bg",
...
def run_e2e(ws_url: str, baselines_root: pathlib.Path) -> None:
    ...
    try:
        ...
        raise E2EFailure(
            ...
            "  Ensure the Godot fixture is running (login scene active)."
        )

    with ws:
        if ws.subprotocol != SUBPROTOCOL:
            raise E2EFailure(
                f"Subprotocol mismatch: expected {SUBPROTOCOL!r}, "
                f"got {ws.subprotocol!r}"
            )
        ...
        if not_pinned:
            raise E2EFailure(f"nodes present but not pinned: {sorted(not_pinned)}")
        ...
        _ok("initial visibility: login_panel=visible, welcome_panel=hidden")

        # ── 4–5. Login flow — enter credentials ───────────────────────────
        _step("4. send_text → account_input_bg")
        rpc(ws, "send_text",
            {"id": "account_input_bg", "text": "admin"}, req_id=3)
        _ok("send_text 'admin' to account_input_bg")

        _step("5. send_text → password_input_bg")
        rpc(ws, "send_text",
            {"id": "password_input_bg", "text": "password"}, req_id=4)
        _ok("send_text 'password' to password_input_bg")

        # ── 6. Click login button ──────────────────────────────────────────
        _step("6. click → login_button_bg")
        rpc(ws, "click", {"id": "login_button_bg"}, req_id=5)
        _ok("click dispatched to login_button_bg")

        time.sleep(0.2)

        # ── 7. Assert post-login state ─────────────────────────────────────
        _step("7. dump_tree — post-login state")
        ...
        # ── 8. compare_screenshot (visual regression) ──────────────────────
        _step("8. compare_screenshot — welcome screen baseline")
        shot_result = rpc(ws, "compare_screenshot",
                          {"name": "godot_welcome_screen", "threshold": 0.95},
                          req_id=7)
        saved_path = (
            shot_result.get("saved_path", "") if isinstance(shot_result, dict) else ""
        )
        if not saved_path:
            raise E2EFailure(f"compare_screenshot returned no saved_path: {shot_result!r}")
        _ok(f"compare_screenshot → {saved_path}")
        ...
        # ── 9. find_widget by role cross-check ────────────────────────────
        ...
        if "login_button_bg" not in buttons:
            raise E2EFailure(
                f"find_widget(button) missed login_button_bg: {buttons}"
            )
...
def _run_ssim_check(
    baselines_root: pathlib.Path,
    saved_path: pathlib.Path,
    name: str,
) -> None:
    baseline = baselines_root / "godot" / "windows" / f"{name}.png"
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="godot_login_e2e",
        description="Full Godot login flow e2e (TASK-0306).",
    )
    p.add_argument(
        "--url", default=DEFAULT_URL,
        help=f"WebSocket URL of the Godot adapter (default: {DEFAULT_URL})",
    )
    p.add_argument(
        "--baselines-root", type=pathlib.Path,
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("AutoAgent — Godot Login e2e (TASK-0306)")
    ...
    try:
        ...
        return 1
    print("\nALL CHECKS PASSED — Godot login flow end-to-end OK.")
    return 0
