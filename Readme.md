...
- **GPU**：NVIDIA RTX 3080Ti 12GB 或更高（运行 UI-TARS VLM）
- **WSL2**：Ubuntu 22.04（运行 vLLM 服务）
...
成功后会在 `./data/evidence/<session_id>/` 下生成报告和截图，
...
| `get_session_status` | 查询会话状态 | `session_id` |
...
AutoVisionTest 调用 UI-TARS 模型，自主决策每一步操作，直到完成目标或达到终止条件。
...
5. 如果 result.status == "FAIL"，读取 result.failure_reason 和 steps[].screenshot_after
   （截图为 base64 PNG，你可以直接查看），分析问题并修复代码后重新测试
...
AI Agent 读到后，查看最后几步截图，发现保存对话框未弹出，定位到代码里 `Ctrl+S` 快捷键写错为 `Ctrl+X`，修复后重测。

---

## 7. HTTP API 接口

适合自定义 Agent 或 CI 流水线集成。
...
└── src/autovisiontest/
    ...
    │   └── assertions.py       # 断言引擎
    ├── backends/
    │   └── uitars.py           # UI-TARS 模型后端
    ├── scheduler/
    │   └── session_scheduler.py # 会话生命周期管理
    └── report/
        └── builder.py          # 报告生成
```
...
首次是探索模式，UI-TARS 需要逐步探索 UI，通常 50 步以内完成（约 2-4 分钟，取决于目标复杂度）。探索成功后自动固化，之后回归模式只需 30-60 秒。

**Q: 测试失败后，AI Agent 需要做什么？**

1. 调用 `get_session_report` 获取报告
2. 查看 `result.failure_reason` 了解失败原因
3. 查看 `steps` 数组末尾的 `screenshot_after`（base64 截图）直接看到失败时的屏幕状态
4. 查看 `assertions` 数组了解哪个断言失败
5. 根据以上信息定位代码问题，修复后重新触发测试
...
当被测应用 UI 发生较大变化（连续多步 VLM 找不到预期元素）时，AutoVisionTest 会自动标记回归用例失效，并退回探索模式重新录制。也可手动调用 `invalidate_recording` 强制失效。
...
启动 HTTP API 服务后，用 Python/Shell 脚本调用 REST API 即可。参考 [第 7.3 节](#73-cicd-集成示例) 的 Python 示例。构建失败（exit code 1）时 CI 自动报错，报告存储在 `data/evidence/` 供事后分析。
